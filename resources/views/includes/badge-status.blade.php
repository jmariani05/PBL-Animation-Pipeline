...
<button class="btn btn-sm rounded-pill btn-outline-info py-0" data-toggle="modal"
    data-target="#modal-{{$name}}{{$item->id}}">
    <span class="small">TODO</span>
</button>
@endif
@if ($status == 'retake')
<button class="btn btn-sm rounded-pill btn-danger py-0" data-toggle="modal" data-target="#modal-{{$name}}{{$item->id}}">
...
<div class="modal fade" id="modal-{{$name}}{{$item->id}}">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="/project-asset/{{$item->id}}" method="POST">
                @csrf
                @method('put')
